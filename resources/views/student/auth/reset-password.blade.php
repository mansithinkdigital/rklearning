...
<head>
    ...
    <title>Reset Password - Rk Institute</title>
...
<body class="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-indigo-50 to-blue-100">
    <div class="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl p-10 md:p-12 border border-blue-50">
        ...
        <form action="{{ route('student.password.update') }}" method="POST" class="space-y-6">
            @csrf
            <div>
                <label class="text-sm font-bold text-slate-700 block mb-2 px-1">New Password</label>
            ...
            <div>
                <label class="text-sm font-bold text-slate-700 block mb-2 px-1">Confirm New Password</label>
                <input type="password" name="password_confirmation" required class="w-full px-6 py-4 rounded-2xl border border-slate-200 focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all" placeholder="••••••••">
            </div>
            <button type="submit" class="w-full bg-blue-600 text-white py-4 rounded-2xl font-bold text-lg hover:bg-blue-700 transition-all shadow-xl shadow-blue-200 mt-4">Reset Password</button>
        </form>
    </div>
</body>
